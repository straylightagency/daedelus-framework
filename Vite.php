...
class Vite
{
    ...
    protected function assetDev(array $entries): string
    {
        ...
        if ( isset( $entries[0] ) && ! $this->isCssFile( $entries[0] ) && ! $this->isJsFile( $entries[0] ) ) {
            return $this->host . $entries[0];
        }
     ...
     * @return bool
     */
    protected function isJsFile( $entry ):bool
    {
        return str_ends_with( $entry, '.js' );
    }
